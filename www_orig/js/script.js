...
$(document).ready(function () {
  ...
  if (haveSelect.length != 0) {
    var startSelect2 = function startSelect2(event) {
      if (this.value !== "") {
        this.parentNode.classList.add('select2--ok');
      }
    };

    var formatDealer = function formatDealer(item) {
      if (!item.id) {
        return item.text;
      }

      var $item = $('<p class="choise-address__dealer">' + item.element.dataset.name + '</p>' + '<p class="choise-address__address">' + item.element.dataset.address + '</p>' + '<img class="choise-address__icons" src="' + item.element.dataset.img + '.svg" alt="Picture">' + '<img class="choise-address__icons-active" src="' + item.element.dataset.img + '-active.svg" alt="Picture">');
      return $item;
    };

    var formatDealerSelection = function formatDealerSelection(item) {
      if (!item.id) {
        return item.text;
      }

      var $item = $('<p class="choise-address__dealer">' + item.element.dataset.name + '</p>' + '<p class="choise-address__address">' + item.element.dataset.address + '</p>' + '<img class="choise-address__icons choise-address__icons--result" src="' + item.element.dataset.img + '-choise.svg" alt="Picture">');
      return $item;
    };
    ...
    $selectArr.on("select2:select", function () {
      this.parentNode.classList.add('select2--ok');
    });
    $selectArr.on("select2:unselect", function () {
      this.parentNode.classList.remove('select2--ok');
    });
    ;
    ;
  }
  ...
  function startSliderChoiseDate() {
    var choiseDateSlider = document.querySelector('#choise-date-slider'); //если есть слайдер и при этом он еще НЕ инициализирован => запускаем слайдер, предварительно вычислив начальный элемент

    if (choiseDateSlider && !choiseDateSlider.classList.contains('slick-initialized')) {
      var findCurrentDate = function findCurrentDate() {
  ...
  function openInfoMastersDates(event) {
    ...
    if (!this.classList.contains('btn-choise-master--disabled')) {
      ...
      setTimeout(function () {
        document.querySelector('#choise-date-slider').classList.remove('choise-date__slider--hide');
      }, 300);
    }
  } //PopUp
  ...
  $('.popup__close').on("click", PopUpHide); //PopUp можно вызвать принудительно, с помощью 
